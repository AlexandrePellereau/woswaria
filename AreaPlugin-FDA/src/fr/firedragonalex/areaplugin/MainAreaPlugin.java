...
import fr.firedragonalex.areaplugin.saveandload.SaveAndLoadAreas;
...
public class MainAreaPlugin extends JavaPlugin {
	...
	private static HashMap<UUID, List<ItemStack>> notEarnItemStacks;
	...
	public static void addItemToEarn(UUID uuid,List<ItemStack> itemstacksToAdd) {
		if (MainAreaPlugin.notEarnItemStacks.containsKey(uuid)) {
			MainAreaPlugin.notEarnItemStacks.get(uuid).addAll(itemstacksToAdd);
		} else {
			MainAreaPlugin.notEarnItemStacks.put(uuid, itemstacksToAdd);
		}
	}
	
	public static Area getAreaByLocation(Location location) {
		for (Area area : MainAreaPlugin.getThis().getAllAreas()) {
			if (area.isInTheArea(location)) {
				return area;
			}
		}
		return null;
